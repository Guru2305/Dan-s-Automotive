...
import { ShieldCheck, HeartHandshake, Award, Users, Compass, CheckSquare, Sparkles } from 'lucide-react';
...
export default function About() {
  ...
  return (
    <section id="about" className="py-20 bg-[#0A0A0A] border-t border-white/10 relative grain-overlay">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b border-white/10 pb-6">
          <div>
            ...
            <h2 className="font-display text-4xl sm:text-6xl text-white uppercase tracking-tight">
              BUILT ON TRUST. <span className="text-[#C21F2E]">RUN BY FAMILY.</span>
            </h2>
          </div>
        ...
        {/* Story Grid: Text + Workshop Image */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 items-center">
          
          {/* Left Text Block */}
          <div className="lg:col-span-6 space-y-6">
            <div className="inline-block px-3 py-1 bg-[#161616] border border-[#C21F2E]/40 font-mono-spec text-xs text-[#C21F2E] uppercase">
            ...
            <div className="space-y-4 text-[#9A9A95] font-sans-body leading-relaxed text-base">
              <p>
                Founded by <strong className="text-white font-semibold">Dan and Jenna</strong> in Sittingbourne, Kent, Dan's Automotive Services started with a simple belief: car repair shouldn't involve stress, confusing jargon, or inflated bills.
              </p>
              <p>
            ...
            <div className="pt-2 grid grid-cols-2 gap-4 font-mono-spec text-xs">
              <div className="p-3 bg-[#121212] border border-white/10 border-l-2 border-l-[#C21F2E]">
                <span className="text-white font-bold block mb-1">PROVEN GOOGLE RATING</span>
                <span className="text-[#9A9A95]">4.8 Stars out of 165+ independent reviews</span>
              </div>
              <div className="p-3 bg-[#121212] border border-white/10 border-l-2 border-l-[#C21F2E]">
                <span className="text-white font-bold block mb-1">100% IN-HOUSE REPAIRS</span>
                <span className="text-[#9A9A95]">All work carried out on our workshop lifts</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-6">
            <div className="crop-bracket-container crop-marks-top crop-marks-bottom bg-[#121212] border border-white/10 p-3 shadow-2xl relative">
              
              <div className="flex justify-between items-center px-3 py-1.5 bg-[#0A0A0A] border-b border-white/10 font-mono-spec text-[11px] text-[#9A9A95] mb-2">
                <span>WORKSHOP FLOOR // NEWINGTON ME9</span>
                <span className="text-[#C21F2E] font-bold">TEAM: DAN, JENNA, MATT</span>
              </div>
              ...
              <div className="relative overflow-hidden aspect-[4/3] border border-white/10">
                ...
                <div className="absolute bottom-4 left-4 right-4 p-3 bg-[#0A0A0A]/90 backdrop-blur-md border border-white/15 font-mono-spec text-xs">
                  <span className="text-[#C21F2E] font-bold block">EQUIPPED FOR ALL MAKES & MODELS</span>
                  <span className="text-[#9A9A95] text-[11px]">From everyday economical hatchbacks to high-performance engines</span>
                </div>
              </div>
        ...
        <div className="mb-16">
          <div className="font-mono-spec text-xs text-[#9A9A95] uppercase tracking-widest mb-4 flex items-center gap-2">
            <Users className="w-4 h-4 text-[#C21F2E]" />
            <span>MEET THE TEAM AT DAN'S AUTOMOTIVE SERVICES</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 hairline-grid border border-white/10 bg-[#121212]">
            {teamMembers.map((member, idx) => (
              <div key={idx} className="bg-[#121212] p-6 hover:bg-[#161616] transition-colors relative">
                <div className="font-mono-spec text-[10px] text-[#C21F2E] mb-2">// CREW 0{idx + 1}</div>
                <h4 className="font-display text-2xl text-white tracking-wide uppercase mb-1">{member.name}</h4>
                <div className="font-mono-spec text-xs text-[#C21F2E] font-semibold mb-3">{member.role}</div>
                <p className="text-xs text-[#9A9A95] font-sans-body leading-relaxed">{member.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="font-mono-spec text-xs text-[#C21F2E] tracking-widest uppercase font-semibold mb-4">
            [ OUR WORKSHOP CODE & PROMISE ]
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 hairline-grid border border-white/10 bg-[#121212]">
            {values.map((v, i) => {
              const Icon = v.icon;
              return (
                <div key={i} className="bg-[#121212] p-6 hover:bg-[#161616] transition-colors flex flex-col justify-between group">
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <span className="font-mono-spec text-xs text-[#C21F2E] font-bold">{v.code}</span>
                      <Icon className="w-5 h-5 text-[#9A9A95] group-hover:text-[#C21F2E] transition-colors" />
                    </div>
                    <h4 className="font-display text-xl text-white uppercase mb-2 tracking-wide">{v.title}</h4>
                    <p className="text-xs text-[#9A9A95] font-sans-body leading-relaxed">{v.desc}</p>
                  </div>
                  <div className="mt-6 pt-3 border-t border-white/5 font-mono-spec text-[10px] text-white/40 group-hover:text-[#C21F2E] transition-colors">
                    GUARANTEED // 100%
                  </div>
                </div>
              );
